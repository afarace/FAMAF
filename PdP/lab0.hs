...
duplica'' :: Num a => [a] -> [a]
duplica'' xs = polGeneralMap xs (2*)   -- Como estamos usando un operador de numeros hay que especificar que 'a' es un numero.

mas1'' :: Num a => [a] -> [a]
mas1'' xs = polGeneralMap xs (+1)
...
polGeneralFilter [] f = []
polGeneralFilter (x:xs) f | f x = x : polGeneralFilter xs f
                          | not (f x) = polGeneralFilter xs f
          
--c)
soloPares' :: [Int] -> [Int]
soloPares' xs = generalFilter xs even
...
generalFold :: [Int] -> (Int -> Int -> Int) -> Int -> Int   -- El tercer argumento es el valor inicial
generalFold [] f z = z
generalFold (x:xs) f z = f x (generalFold xs f z)

--b)

polGeneralFold :: [a] -> (a -> a -> a) -> a -> a
polGeneralFold [] f z = z
polGeneralFold (x:xs) f z = f x (polGeneralFold xs f z)

--c)

sumatoria' :: [Int] -> Int
sumatoria' xs = generalFold xs (+) 0
...
evaluar (Resta a b) = (evaluar a) - (evaluar b)
evaluar (Division a b) = (evaluar a) `div` (evaluar b)
...
profundidad (Node _) = 1
profundidad (Tree a b) = 1 + max (profundidad a) (profundidad b)
...
genFoldBinTree :: (a -> b) -> (b -> b -> b) -> BinTree a -> b
genFoldBinTree f g (Node x) = f x
genFoldBinTree f g (Tree a b) = g (genFoldBinTree f g a) (genFoldBinTree f g b)

profundidad' :: BinTree a -> Int
profundidad' = genFoldBinTree (\_ -> 1) (\x y -> 1 + max x y)
...
listaBinTree :: BinTree a -> [a]
listaBinTree (Node x) = [x]
listaBinTree (Tree a b) = listaBinTree a ++ listaBinTree b

-- Definición de la función fold
listaBinTree' :: BinTree a -> [a]
listaBinTree' = genFoldBinTree (\x -> [x]) (\xs ys -> xs ++ ys)
