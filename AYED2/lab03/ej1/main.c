...
int main(int argc, char *argv[])
{
    ...
    /* show the lower minimum temperature */
    printf("Lower minimum temperature: %d\n", lower_min_temp(array));

    /* show the maximum temperature by year */
    int max_temp[YEARS];
    max_temp_by_year(array, max_temp);
    for (unsigned int year = 0u; year < YEARS; ++year)
    {
        printf("Max temperature in year %u: %d\n", year + FST_YEAR, max_temp[year]);
    }

    /* show the month with maximum rainfall by year */
    month_t max_rainfall[YEARS];
    max_rainfall_month(array, max_rainfall);
    for (unsigned int year = 0u; year < YEARS; ++year)
    {
        printf("Max rainfall in year %u: %u\n", year + FST_YEAR, max_rainfall[year] + 1);
    }
