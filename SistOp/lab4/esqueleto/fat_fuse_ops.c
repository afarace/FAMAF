...
void fat_fuse_log_activity(char *operation_type, fat_file target_file)
{
    ...
    fat_file fs_log = fat_tree_search(vol->file_tree, path);
    int message_size = strlen(buf);
    size_t size = message_size * sizeof(char);
    fat_tree_node log_node = fat_tree_node_search(vol->file_tree, path);
    fat_file parent = fat_tree_get_parent(log_node);

    /* Se escribe en el fs.log la operacion realizada */
 
    fat_file_pwrite(fs_log, buf, size, fs_log->dentry->file_size, parent);

}

void fat_fuse_mk_log_file() {
    fat_file fs_log;
    fat_volume vol;
    ...
    vol = get_fat_volume();
    fs_log = fat_tree_search(vol->file_tree, path);

    /* Verifica si est� creado /fs.log */
    if (fs_log != NULL) {
    ...
    if (mknod_exit == -ENOENT || mknod_exit == -ENOTDIR) {
        printf("Error al crear archivo log\n");
    }
...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    if (!fat_tree_search(vol->file_tree, LOG_FILE)) {
        fat_fuse_mk_log_file();
    }
}
...
int fat_fuse_rmdir(const char *path){
    ...
    if (!fat_file_is_empty_dir(dir)) {
        return -ENOTEMPTY;
    }
    */
    fat_file_read_children(dir);
    if(dir->dir.nentries > 2)
        return -ENOTEMPTY;
