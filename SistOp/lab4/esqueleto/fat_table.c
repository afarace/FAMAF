...
u32 fat_table_add_new_cluster_to_chain(fat_table table, u32 last_cluster) {
    // Obtener un nuevo clúster libre de la tabla FAT
    u32 new_cluster = fat_table_get_next_free_cluster(table);
    // Verificar si no hay clústeres libres disponibles
    if (fat_table_is_EOC(table, new_cluster)) {
    // Si no hay clústeres libres, retornar el marcador de final de cadena
    ...
    // Retornar el número del nuevo clúster asignado
    return new_cluster;
}
