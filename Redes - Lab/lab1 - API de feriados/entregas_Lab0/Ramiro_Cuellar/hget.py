...
def connect_to_server(server_name):
    ...
    ip_address = socket.gethostbyname(server_name)           # Obtengo la ip del servidor y la asigno a ip_address

    # DEJAR LA LINEA SIGUIENTE TAL COMO ESTA
    sys.stderr.write("Contactando al servidor en %s...\n" % ip_address)

    # COMPLETAR ABAJO DE ESTA LINEA
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)     # Crea el socket
    s.connect((ip_address, HTTP_PORT))                       # Conecto al puerto HTTP_PORT (80)
