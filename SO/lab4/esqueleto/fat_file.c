...
static void write_dir_entry(fat_file parent, fat_dir_entry child_disk_entry,
    ...
    /* PARTE 2 */
    if(is_log_file_dentry(child_disk_entry->base_name, child_disk_entry->extension)){
        memmove(child_disk_entry->base_name+1,child_disk_entry->base_name,7);
        /* Marca el archivo como pendiente para ser eliminado */
        child_disk_entry->base_name[0] = FAT_FILENAME_DELETED_CHAR;
        /* Indica que la entrada no debe ser modificada */
...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        if(is_log_file_dentry((disk_dentry_ptr->base_name+1),disk_dentry_ptr->extension)){  // Asegura que no se ignore la entrada del archivo fs.log
            memmove(disk_dentry_ptr->base_name,disk_dentry_ptr->base_name+1,7);             //Quita el caracter DELETED del archivo fs.log
...
void fat_file_delete(fat_file file, fat_file parent) {
    ...
    u32 next_cluster = 0;
    //Marcamos como libres los clusters incluido el primero
    
    // (last_cluster == EOC) ==> stop
    while (!fat_table_is_EOC(file->table, last_cluster) ) {
        next_cluster = fat_table_get_next_cluster(file->table, last_cluster);
        fat_table_set_next_cluster(file->table, last_cluster, FAT_CLUSTER_FREE);
        last_cluster = next_cluster;
    ...
    //fat_table_set_next_cluster(file->table, last_cluster, FAT_CLUSTER_FREE);
    // Marcamos como borrado su dentry padre
    file->dentry->base_name[0] = FAT_FILENAME_DELETED_CHAR;
    write_dir_entry(parent, file->dentry, file->pos_in_parent );
} 

bool fat_file_is_empty_dir(fat_file dir){
    GList *tmp = fat_file_read_children(dir);
    return ( g_list_length(tmp)==0 );
}
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    if (fat_table_is_EOC(file->table, cluster)) {
        // Si todos los clústeres están llenos, buscamos el último clúster
        ...
        // Después de obtener el último clúster, agregamos un nuevo clúster libre a continuación
