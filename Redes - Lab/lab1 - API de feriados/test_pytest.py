...
def mock_response():
    with requests_mock.Mocker() as m:
        ...
        #PARTE 4
        m.get('http://localhost:5000/peliculas/lista/genero/Acción', status_code=200)

        m.get('http://localhost:5000/peliculas/lista/titulo/Star', status_code=200)

        m.get('http://localhost:5000/peliculas/sugerir', status_code=200)

        m.get('http://localhost:5000/peliculas/sugerir/Acción', status_code=200)

        m.get('http://localhost:5000/peliculas/feriado/Acción', status_code=200)
...
# PARTE 4
def test_obtener_lista_genero(mock_response):
    response = requests.get('http://localhost:5000/peliculas/lista/genero/Acción')
    assert response.status_code == 200
    lista_genero = response.json()
    print({lista_genero})
    assert lista_genero[0]['genero'] =='Acción'


def test_obtener_lista_titulo(mock_response):
    response = requests.get('http://localhost:5000/peliculas/lista/titulo/Star')
    assert response.status_code == 200
    lista_titulo = response.json()
    assert 'Star' in lista_titulo[0]['titulo']
...
def test_sugerir_pelicula_aleatoria_genero(mock_response):
...
def test_sugerir_pelicula_aleatoria(mock_response):
    ...
    assert response.status_code == 200
    assert "fecha" in str(response.json())
    assert "motivo" in str(response.json())
    assert "pelicula" in str(response.json())
